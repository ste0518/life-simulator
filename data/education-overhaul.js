(function () {
  ...
  const GRADUATION_EVENTS = [
    ...
    event({
      ...
      text: "出国以后，原来的语言环境、消费习惯、社交方式和感情节奏都会一起被换掉。你会得到新的城市、新朋友圈和新对象池，也会同时面对更直接的经济压力、孤独感和文化差异。\n\n钱会先说话：家里能掏多少、你自己攒了多少、能不能一次性押出一笔首期开销，会决定你是现款上路还是贷款把未来先抵押出去。贷款不是轻飘飘的一句设定，它会变成负债、海外经济压力、生活方式上限，以及后面工作与情绪里长期的背景噪音。\n\n若现金和家庭支持都接不住，也达不到贷款所需的基础条件，你只能把出国从这一站拿下来，改走国内路线。\n\n系统会按你现在的学业能力列出海外学校参考（QS 档位），不等于录取承诺。若国内已经有人在你心里占了位置，这段关系只会被拖进更难维系的状态里。\n\n{studyLoanSummary}",
      minAge: 18,
      maxAge: 21,
      weight: 100,
      ...
      choices: [
        ...
        choice({
          text: "把出国先放回抽屉：现金和贷款都够不着，只能回头走高考那条路。",
          customAction: "abandon_overseas_for_domestic",
          customPayload: {
            targetLifePath: "gaokao",
            log: "你把出国从当下人生里先拿下来，回头去挤高考与志愿的那扇门。"
          },
          next: "score_and_volunteer"
        }),
        choice({
          text: "承认家里接不住，也读不下去考试，只能改走不高考的现实入口。",
          customAction: "abandon_overseas_for_domestic",
          customPayload: {
            targetLifePath: "non_gaokao",
            log: "出国落空以后，你转向另一条更粗粝的近路：不高考，直接去社会里找立足点。"
          },
          next: "non_gaokao_departure"
        })
      ]
