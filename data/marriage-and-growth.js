(function () {
  ...
  const defaultStageProfiles = {
    ...
    graduate_transition:
      "{name}刚离开校园或站在毕业边缘：简历、面试、比较与自我怀疑一起涌上来。Ta 常挂在嘴边的不再是哪门课，而是「先活下来」与「别掉队」；对感情要么更急，要么更不敢承诺。",
    work_early:
      "{name}已经把自己嵌进某座城市的通勤里：早高峰、外卖、房租提醒与周报并排躺在手机里。浪漫还在，但经常被加班、社交应酬和「下次再说」挤到后排；Ta 学会用消费和短暂放松补偿透支，也开始用存款与职位名称为自己辩护。",
    young_adult_deep:
      "{name}在「稳定下来」和「再拼一把」之间来回摆：一边羡慕别人的确定，一边又怕确定意味着认输。对关系的耐心与钱包厚度意外绑定——不是嫌你穷，而是怕两个人一起扛不住现实；对未来嘴上说着随缘，心里却在算风险。",
    married: "{name}把「我们」写进日常开支表和家务分工里，亲密不再只是心动，而是能不能一起把日子过顺。",
    mature:
      "{name}更清楚自己要什么、能忍什么：有些温柔变沉了，有些棱角也被现实磨钝或磨尖。对感情不再迷信戏剧性，更看重「出事时会不会在场」；谈起未来会更直接地谈钱、谈健康、谈分工，也会更警惕空头承诺。"
  };
  ...
  const characterStageProfiles = {
    ...
    song_qinghe: {
      ...
      graduate_transition:
        "{name}在毕业与求职的缝里把日子过成清单：改简历、测评、群面、沉默的已读不回。她仍会把情绪写进备忘录，但语气比以前更硬——像在说服自己别垮。对约会地点开始算交通成本，对「以后」这个词既渴望又警惕。",
      work_early:
        "{name}多半落在需要「把话说清楚」的岗位：新媒体运营、品牌文案、教育内容或出版周边。她在上海或杭州的写字楼里改第 17 版海报文案，也在深夜地铁上回完最后一条工作消息。朋友圈比以前更像精选栏目；对你，她会更直接地问「你到底能不能稳定」，不是嫌弃，是怕两个人一起被生活碾碎。",
      young_adult_deep:
        "{name}开始把温柔换成可执行的照顾：记得你体检、催你睡觉、帮你把焦虑拆成步骤。她不再迷信轰轰烈烈的表白，更在意「出事时你会不会接电话」。若你走得太野，她不会大吵，只会慢慢把期待收回去，收到你察觉不到的地方。",
      married: "{name}会把婚姻当成长期文本：需要修订、批注和共同署名，而不是一次性的誓言。",
      mature:
        "{name}更会把情绪翻译成责任：孩子、父母、贷款与体面并排放在心里。她可能更少说自己怕什么，但会用更沉默的方式需要你靠近；对感情的底线变成「别让我一个人扛完所有具体的事」。",
      byArc: {
        cold_success: {
          work_early:
            "{name}升得很快，谈吐更利，回消息越来越像在工作群里留痕。她在一线大厂做内容或运营，会议纪要比情书还熟；你见她笑，多半是社交性的。",
    ...
    jiang_xun: {
      ...
      graduate_transition:
        "{name}把「先找到事做」挂在嘴边：白天跑面试，晚上陪兄弟喝酒吐槽。他不喜欢被问太细，但你会看见他手机屏保从球队合影换成招聘软件。对感情更嘴硬——怕显得怂，也怕给不出确定答案。",
      work_early:
        "{name}多半扎进销售、渠道、场馆运营或任何「靠把人搞定」吃饭的行当。本地或省会城市的酒局与业绩表轮流教他做人；他说「没事」的时候，通常胃和睡眠已经先有事。恋爱里他更习惯用行动买单，但也会把你的担心误判成不信任。",
      young_adult_deep:
        "{name}开始把「值不值」问出口，却常常问完还是去冲。他想要被理解成一个还在长的人，而不是只会扛的人；你若只夸他硬，他会高兴，也会更孤独。",
    ...
    lin_xiaonan: {
      youth: defaultStageProfiles.youth,
      ...
      graduate_transition:
        "{name}把未来当成可优化问题：保研、出国、实习、考公，每条路都被她算过期望与方差。她不说怕，但会突然在深夜问你「我是不是选错了」——问完又后悔示弱。",
      work_early:
        "{name}更常出现在实验室、数据岗、制造业工程师或金融机构的后台岗位：工装或白衬衫，咖啡与黑眼圈。她习惯用「还行」总结一切，却把压力攒在肩颈。对感情，她要的不是甜言蜜语，而是你愿意陪她一起把现实算清楚。",
      young_adult_deep:
        "{name}开始承认自己也会累，但承认得很克制。她可能会为了关系调整职业节奏，也会因此偷偷不甘；你若把她的牺牲当理所当然，她会用冷静离开而不是哭闹。",
    ...
    zhou_mingyue: {
      youth: defaultStageProfiles.youth,
      adolescence: defaultStageProfiles.adolescence,
      highschool: "{name}在广播站和人群之间切换自如，嘴快、好笑，也更怕被看穿软肋。",
      college: "{name}如果去了媒体、艺术或外向型专业，会如鱼得水；若被摁在不适合的赛道，会蔫也会炸。",
      graduate_transition:
        "{name}把「机会」当成硬通货：实习内推、跨界项目、短视频与展演场地都能成为她的跳板。她看起来很敢，也会在凌晨删朋友圈——删的不是文案，是怕被你看见太想要。",
      work_early:
        "{name}更可能落在创意热店、直播电商、演艺经纪或公关公司：表面光鲜，背后是排期、撕扯与不确定收入。她的社交圈比以前更杂，也更擅长用玩笑挡真心；你若只想「照顾她」，她会觉得被小看。",
      young_adult_deep:
        "{name}开始区分「喜欢」和「能并肩」：她要的是两个人都能站在光里，而不是谁拖着谁。她对婚姻不迷信，但很在意尊重；家长反对时，她会撕裂，也会逼你做选择。",
